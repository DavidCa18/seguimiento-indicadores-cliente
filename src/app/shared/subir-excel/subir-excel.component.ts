...
export class SubirExcelComponent implements OnInit {
  ...
  progressAlert(evt: any) {
    ...
    Swal.fire({
      ...
      if (result.dismiss === Swal.DismissReason.timer) {
        if ( this.excelSubir. length == 0 && this.excelMalIdentificado. length == 0 ) {
          Swal.fire(
            'Algo esta Mal',
  ...
  async ingresarDatosIndicador(element) {
    ...
    if (this.headerValidar(element, 'UNIDAD /')) {
      indicador.unidad = this.headerValidar(element, 'UNIDAD /');
    }
    if (this.headerValidar(element, 'REPORTA')) {
      indicador.responde = this.headerValidar(element, 'REPORTA');
    }
    ...
    if (this.headerValidar(element, 'MACRO PROCESO')) {
      indicador.macroProceso = this.headerValidar(element, 'MACRO PROCESO');
    }
  ...
  async addPeriod(quantity: number, indicador: Indicadores) {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      //Assign Results
      // if (indicador.periods.length > 0) {
      //   const periodFromGroup = {
      //     goal: this.metas[i],
      //     result: indicador.periods[i].result,
      //     compliance: indicador.periods[i].compliance,
      //     class: indicador.periods[i].class,
      //     color: indicador.periods[i].color,
      //     enabled: indicador.periods[i].enabled,
      //     solicitud: indicador.periods[i].solicitud,
      //   };
  ...
  //Descargar
  descargarPlantilla() {
    this._indicadorService.descargar(
