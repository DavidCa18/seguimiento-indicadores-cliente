...
export class IndicadoresGestionComponent implements OnInit {
  ...
  crearInidcador() {
    ...
    this.registerForm.value.estadoSolicitud = "NULA";
    this.registerForm.value.autorizacion = "";
    var temp1 = this.unidades.find(e => e.unidad == this.registerForm.value.unidad);
    this.registerForm.value.role = temp1 == undefined ? '' : temp1.role;

    this.registerForm.value.fechaCreacion = new Date(new Date().getTime() -  ( new Date().getTimezoneOffset() * 60000 ));
    this.registerForm.value.fechaModificacion = new Date(new Date().getTime() -  ( new Date().getTimezoneOffset() * 60000 ));
