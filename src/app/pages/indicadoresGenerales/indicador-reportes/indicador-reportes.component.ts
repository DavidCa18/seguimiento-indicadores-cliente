...
export class IndicadorReportesComponent implements OnInit {
  ...
  getUnidades() {
    this._filtroIndicador.get('indicadores/unidades').subscribe((resp) => {
      this.unidades = resp;
    });
  }
  ...
  async generatePDF(data, total) {
    let docDefinition = {
      content: [
        ...
        {
          ul: [
            `Unidad: ${this.filtroForm.value.unidad}`,
          ...
          table: {
            ...
            body: [
              ...
              ...data.map((p) => [
                ...
                fillOpacity: 0.6, fillColor: this.colorCumplimiento(p.cumplimientoIndicador) },
              ]),
            ],
          },
          layout: {
  ...
  async generateExcel() {
    ...
    const footerRow = worksheet.addRow([
      `Los Indicadores son según ( Departamento: ${this.filtroForm.value.unidad} & Desde: ${this.filtroForm.value.desde} - Hasta: ${this.filtroForm.value.hasta}  )`,
    ]);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      fs.saveAs(
        blob,
        `Reporte-${this.filtroForm.value.unidad}(${this.filtroForm.value.desde})-(${this.filtroForm.value.hasta}).xlsx`
      );
    });
  }
}
